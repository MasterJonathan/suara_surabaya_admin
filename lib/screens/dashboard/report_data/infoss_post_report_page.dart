// lib/screens/dashboard/report_data/infoss_post_report_page.dart
...
class InfossPostReportPage extends StatefulWidget {
  const InfossPostReportPage({super.key});

  @override
  State<InfossPostReportPage> createState() => _InfossPostReportPageState();
}

class _InfossPostReportPageState extends State<InfossPostReportPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<InfossPostReportProvider>().generateReport(
        _startDate,
        _endDate,
      );
    });
  ...
  void _exportToCsv(InfossPostReportProvider provider) {
  ...
  Widget build(BuildContext context) {
    final provider = context.watch<InfossPostReportProvider>();
            ...
            child: Row(
              ...
              children: [
                Column(
                  ...
                  children: [
                    Text(
                      'Laporan Posting Info SS',
                      style: Theme.of(context).textTheme.headlineSmall
                          ?.copyWith(fontWeight: FontWeight.bold),
                    ),
                    const SizedBox(height: 4),
                    Text(
                      'Analisis jumlah postingan berita per hari',
                      style: Theme.of(
                        context,
            ...
            Padding(
              ...
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 16), // Jarak vertikal antar baris
                  // Baris Kedua (Bawah)
                  Row(
                    children: [
                      Expanded(
          ...
          if (provider.state != ReportViewState.Busy &&
            ...
            CustomCard(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    'Top 10 Info SS (Berdasarkan Views)',
  ...
  Widget _buildDailyDataTable(List<DailyInfossReport> data) {
    return DataTable(
  ...
  Widget _buildTopPostsTable(List<InfossModel> posts) {
    return DataTable(
      headingRowColor: MaterialStateProperty.all(AppColors.primary),
      columns: const [
        DataColumn(
          label: Text('Judul', style: TextStyle(fontWeight: FontWeight.bold)),
        ),
        DataColumn(
          label: Text(
            'Kategori',
        ...
        DataColumn(
          label: Text('Views', style: TextStyle(fontWeight: FontWeight.bold)),
        ),
        DataColumn(
            ...
            return DataRow(
              cells: [
                DataCell(
                  SizedBox(
                    width: 300,
                    child: Text(post.judul, overflow: TextOverflow.ellipsis),
                  ),
                ),
                DataCell(Text(post.kategori)),
                DataCell(
                  Text(DateFormat('dd/MM/yy HH:mm').format(post.uploadDate)),
                ),
                DataCell(
                  Text(
                    post.jumlahView.toString(),
                    style: const TextStyle(
                      fontWeight: FontWeight.bold,
                      color: Colors.purple,
                    ),
                ...
                DataCell(Text(post.jumlahLike.toString())),
              ],
            );
          }).toList(),
    );
