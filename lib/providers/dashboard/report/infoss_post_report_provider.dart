// lib/providers/dashboard/report/infoss_post_report_provider.dart
...
class DailyInfossReport {
  final DateTime date;
  final int count;
  DailyInfossReport({required this.date, required this.count});
}

enum ReportViewState { Idle, Busy }

class InfossPostReportProvider extends ChangeNotifier {
  final FirestoreService _firestoreService;

  List<DailyInfossReport> _reportData = [];
  List<InfossModel> _topPosts = []; // Top 10 post berdasarkan View
  ...
  // Getters
  List<DailyInfossReport> get reportData => _reportData;
  ...
  InfossPostReportProvider({required FirestoreService firestoreService})
  ...
  Future<void> generateReport(DateTime startDate, DateTime endDate) async {
    ...
    try {
      ...
      final posts = await _firestoreService.getInfossInDateRange(startDate, adjustedEndDate);
      ...
      for (final post in posts) {
        ...
        _totalViews += post.jumlahView;
        _totalLikes += post.jumlahLike;
        _totalComments += post.jumlahComment;
      }

      _reportData = dailyCounts.entries.map((entry) {
        return DailyInfossReport(date: entry.key, count: entry.value);
      }).toList();
      
      _reportData.sort((a, b) => a.date.compareTo(b.date));

      // Siapkan Top 10 Posts (Berdasarkan View)
      _topPosts = List.from(posts);
      _topPosts.sort((a, b) => b.jumlahView.compareTo(a.jumlahView)); 
